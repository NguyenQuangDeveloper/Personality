...
public partial class App : System.Windows.Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        var callerArg = e.Args.FirstOrDefault(arg => arg.StartsWith(Application.LoggerEngine.CALLER_ARG))?.Split("=");
        if (callerArg != null && callerArg.Length == 2)
        {
            CallerAppName = callerArg[1];
        }

        var logFolderPathArg = e.Args.FirstOrDefault(a => a.StartsWith(Application.LoggerEngine.LOG_FOLDER_PATH_ARG))?.Split("=");
        if (logFolderPathArg != null && logFolderPathArg.Length == 2)
        {
            LogFolderPath = logFolderPathArg[1];
        }

        _appMutex = new Mutex(true, $"{Application.LoggerEngine.APP_NAME}_{CallerAppName}", out var createdNew);
        if (!createdNew)
        {
            _appMutex.Dispose();
            Current.Shutdown();
        ...
        {
            Application.LoggerEngine.NotifyReady(CallerAppName);
        }
